...
export default function SignupPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      const error = err instanceof Error ? err : new Error("Google signup failed");
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="glass rounded-[2.5rem] border border-indigo-100 shadow-2xl p-8 md:p-12">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-4 bg-white border border-indigo-100 rounded-2xl text-sm font-bold text-indigo-900/70 hover:bg-indigo-50 hover:border-indigo-200 transition-all shadow-sm disabled:opacity-60 mb-6"
          >
            {googlePending ? (
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-4 bg-white text-indigo-900/30 font-black uppercase tracking-widest">Or sign up with email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  suppressHydrationWarning
                  className={cn("w-full pl-12 pr-5 py-4 bg-indigo-50/50 border rounded-2xl text-sm font-bold text-indigo-950 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all placeholder:text-indigo-200", fieldErrors.name ? "border-rose-400" : "border-indigo-100")}
                />
              </div>
              {fieldErrors.name && <p className="text-[11px] text-rose-500 font-bold ml-1">{fieldErrors.name}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              {!fieldErrors.password && (
                <p className="text-[11px] text-indigo-900/30 font-bold ml-1">Min 6 chars, must include a letter and a number</p>
              )}
            </div>
          ...
          <p className="mt-8 text-center text-sm font-bold text-indigo-900/40">
